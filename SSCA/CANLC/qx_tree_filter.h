...
class qx_tree_filter {
 public:
  ...
  int init(int h, int w, int nr_channel, double sigma_range = QX_DEF_MST_KI_SIGMA_RANGE,
           int nr_neighbor = QX_DEF_MST_KI_4NR_NEIGHBOR);

  int build_tree(unsigned char *texture);

  //void init_tree_value(unsigned char*image);
  template<typename T>
  void init_tree_value(T *image, bool compute_weight);

  template<typename T>
  void combine_tree(T *image_filtered);
