...
void gaussian_recursive_x(double **od, double **id, int w, int h, double a0, double a1, double a2, double a3, double b1,
                          double b2, double coefp, double coefn);

void gaussian_recursive_y(double **od, double **id, int w, int h, double a0, double a1, double a2, double a3, double b1,
                          double b2, double coefp, double coefn);
...
inline unsigned char qx_max_u3(unsigned char r, unsigned char g, unsigned char b) {
  return (uint8_t) fmax(fmax(r, g), b);
}
...
inline void image_zero(unsigned char **in, int h, int w, unsigned char zero = 0) {
  memset(in[0], zero, sizeof(unsigned char) * h * w);
}
...
inline unsigned char rgb_2_gray(unsigned char *in) {
  return (uint8_t) (0.299 * in[0] + 0.587 * in[1] + 0.114 * in[2] + 0.5);
}
...
void qx_specular_free_image(unsigned char ***image_specular_free, unsigned char ***image_normalized,
                            float **diffuse_chromaticity_max, int h, int w);
...
inline void image_copy(double ***out, double ***in, int h, int w, int d) {
  memcpy(out[0][0], in[0][0], sizeof(double) * h * w * d);
}

inline void image_copy(unsigned char **out, unsigned char **in, int h, int w) {
  memcpy(out[0], in[0], sizeof(unsigned char) * h * w);
}
...
void qx_detect_occlusion_left_right(unsigned char **mask_left, unsigned char **depth_left, unsigned char **depth_right,
                                    int h, int w, int nr_plane);
