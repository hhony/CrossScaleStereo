...
int qx_nonlocal_cost_aggregation::init(int h, int w, int nr_plane, double sigma_range, double max_color_difference,
                                       double max_gradient_difference, double weight_on_color) {
  clean();
  m_h = h;
  m_w = w;
  m_nr_plane = nr_plane;
  m_sigma_range = sigma_range;
  m_max_color_difference = max_color_difference;
  m_max_gradient_difference = max_gradient_difference;
...
void
qx_nonlocal_cost_aggregation::matching_cost_from_color_and_gradient(unsigned char ***left, unsigned char ***right) {
  image_zero(m_cost_vol, m_h, m_w, m_nr_plane);
  ...
  for (int i = 0; i < m_nr_plane; i++) {
    ...
    for (int y = 0; y < m_h; y++)
      for (int x = 0; x < (m_w); x++) {
        ...
        double cost_gradient = min((double) abs(m_gradient_left[y][x] - m_gradient_shifted[y][x]),
                                   m_max_gradient_difference);
