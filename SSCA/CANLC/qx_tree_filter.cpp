...
void qx_tree_filter::update_table(double sigma_range) {
  ...
  for (int i = 0; i <= QX_DEF_CHAR_MAX; i++)
    m_table[i] = exp(-double(i) / (QX_DEF_CHAR_MAX * sigma_range));//weight table
}
...
template<typename T>
void qx_tree_filter::combine_tree(T *image_filtered) {
...
template<typename T>
void qx_tree_filter::init_tree_value(T *image, bool compute_weight) {
...
int qx_tree_filter::filter(double *cost, double *cost_backup, int nr_plane) {
  ...
  for (int i = 1; i <
                  m_nr_pixel; i++)//K_00=f(0,00)[K_0-f(0,00)J_00]+J_00, K_00: new value, J_00: old value, K_0: new value of K_00's parent
  {
    int id = *node_id0++;
    int id_ = id * nr_plane;
