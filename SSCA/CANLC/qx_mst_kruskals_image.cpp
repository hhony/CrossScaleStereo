...
inline int qx_mst_compute_nr_edge_4neighbor(int h, int w) {
  if (h <= 2 && w <= 2) return (0);
  else
    return ((h - 1) * w + (w - 1) * h);
}

inline int qx_mst_compute_nr_edge_8neighbor(int h, int w) {
  if (h <= 2 && w <= 2) return (0);
  else
...
inline void
qx_mst_compute_edges_per_pixel(int **edges, unsigned char *distance, unsigned char *image, int nr_channel, int &nr_edge,
                               int y0, int x0, int yt, int xt, int h, int w) {
...
inline int
qx_mst_compute_edges_4neighbor(int **edges, unsigned char *distance, unsigned char *image, int nr_channel, int h,
                               int w) {
  int y0, yt, x0, xt;
  int nr_edge = 0;
  for (y0 = 0; y0 < h; y0++) {
...
inline int
qx_mst_compute_edges_8neighbor(int **edges, unsigned char *distance, unsigned char *image, int nr_channel, int h,
                               int w) {
  int y0, yt, x0, xt;
...
void qx_mst_kruskals_image::clean() {
  ...
  qx_freei(m_edge);
  m_edge = NULL;
  qx_freeu_1(m_distance);
  m_distance = NULL;
  qx_freei_1(m_id_edge);
  m_id_edge = NULL;

  qx_freei_1(m_parent);
  m_parent = NULL;
  qx_freei_1(m_nr_child);
  m_nr_child = NULL;
  qx_freeu_1(m_weight);
  m_weight = NULL;
  qx_freei(m_children);
  ...
  qx_freei(m_connected);
  m_connected = NULL;
  qx_freeu(m_connected_distance);
  m_connected_distance = NULL;
  qx_freei_1(m_nr_connected);
  m_nr_connected = NULL;

  qx_freei_1(m_node_id_from_parent_to_child);
  m_node_id_from_parent_to_child = NULL;
  qx_freei_1(m_rank);
  m_rank = NULL;
}
...
int qx_mst_kruskals_image::init(int h, int w, int nr_channel, int nr_neighbor) {
  ...
  m_h = h;
  m_w = w;
  m_nr_channel = nr_channel;
  m_nr_neighbor = nr_neighbor;
...
//	timer.start();
  ...
  if (m_nr_neighbor == QX_DEF_MST_KI_4NR_NEIGHBOR)
    qx_mst_compute_edges_4neighbor(m_edge, m_distance, m_image, m_nr_channel, m_h, m_w);//find edges
