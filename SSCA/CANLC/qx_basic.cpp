...
void
gaussian_recursive_x(float **od, float **id, int w, int h, float a0, float a1, float a2, float a3, float b1, float b2,
...
void
gaussian_recursive_y(float **od, float **id, int w, int h, float a0, float a1, float a2, float a3, float b1, float b2,
                     float coefp, float coefn) {
  float xp = 0.0f;  // previous input
  float yp = 0.0f;  // previous output
  float yb = 0.0f;  // previous output by 2
...
void qx_specular_free_image(unsigned char ***image_specular_free, unsigned char ***image_normalized,
  ...
  int y, x;
  unsigned char *image_specular_free_x, *image_normalized_x;
  float *diffuse_chromaticity_max_x;
  unsigned char r, g, b;
  double imax, isum;
  float rf, gf, bf, c, t0, t1, t2, t3, diffuse, specular;
...
void depth_best_cost(unsigned char **depth, double ***evidence, int h, int w, int nr_planes) {
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      int d;
      vec_min_pos(d, evidence[y][x], nr_planes);
      depth[y][x] = d;
    }
}
